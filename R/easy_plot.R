easy_plot <- function(Data,y_col,x_col,dataformating,unprocessed_type='plot_unprocessed_line',
                      reshaping_list=NULL,gen_aes_list,perform_reshape=FALSE,plot_parameters=NULL,get_current_theme=NULL,...){
  # the philosophy:
  ...
  if (!is.null(get_current_theme)){
    get_current_theme <- get_theme(get_current_theme)
    
    
    
  } else {
    get_current_theme <-get_theme()
  ...
  switch(dataformating,
         processed_in_long = {
           ...
           p + get_current_theme+plot_parameters[["limits"]]
